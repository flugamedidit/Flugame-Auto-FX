...
export default async function handler(req, res) {
  ...
  try {
    const generated = await executeGeneration({
      ...
      fallbackModel: process.env.GOOGLE_VIDEO_MODEL,
      fallbackEndpoint: process.env.GOOGLE_VIDEO_API_URL,
    });
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : 'Video generation provider returned an error',
    });
  }
}
